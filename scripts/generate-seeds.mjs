...
// Timestamps are emitted as "now - offset" so seeded activity always looks recent;
// the offsets themselves are deterministic (fixed rng seed) → reproducible files.
...
out.push(`INSERT OR IGNORE INTO users (id,name,email,email_verified,created_at,updated_at) VALUES\n` +
  userRows.map(u => `(${q(u[0])},${q(u[1])},${q(u[2])},0,${NOW} - 2592000,${NOW} - 2592000)`).join(',\n') + ';')

out.push(`INSERT OR IGNORE INTO cinemas (id,name,address,city,latitude,longitude,created_at) VALUES\n` +
  CINEMAS.map(c => `(${q(c.id)},${q(c.name)},${q(c.address)},${q(c.city)},${c.lat},${c.lng},${NOW} - 2592000)`).join(',\n') + ';')
...
out.push(`INSERT OR IGNORE INTO movies (id,title,language,duration_min,hue,emoji,created_at) VALUES\n` +
  [...movieIds.entries()].map(([title, id]) => {
    ...
    return `(${q(id)},${q(m.title)},${q(m.language)},${m.durationMin},${m.hue},${q(m.emoji)},${NOW} - 2592000)`
  }).join(',\n') + ';')
...
for (const s of showRows) for (let i = 0; i < s.reports; i++) {
  ...
  adRows.push(`(${q(`a${adRows.length + 1}`)},${q(`u${101 + (adRows.length * 7) % 80}`)},${q(s.cinemaId)},${q(s.movieId)},${q(s.id)},${Math.max(1, Math.round(s.adMin + jitter))},${NOW} - ${Math.floor(rng() * 2592000)})`)
}
...
for (const c of CINEMAS) {
  for (let i = 0; i < c.ratingCount; i++) {
    ...
    rateRows.push(`(${q(`rt${rateRows.length + 1}`)},${q(uid)},${q(c.id)},${overall},${vals.join(',')},${rev ? q(rev[1]) : 'NULL'},${NOW} - ${(c.ratingCount - i) * 40000})`)
  }
}
